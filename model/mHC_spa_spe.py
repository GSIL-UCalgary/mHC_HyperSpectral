...
class HyperConnection(nn.Module):
    ...
    def width_connection(self, h: torch.Tensor):
        ...
        return mix_h, beta_constrained, alpha
...
class SpaImageHyperConnectionBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor, mask=None) -> torch.Tensor:
        mix_h_attn, beta_attn, alpha = self.attn_hc.width_connection(h)
        h_in_attn = self.attn_norm(mix_h_attn[..., 0, :])
        #attn_output, _ = self.attention(h_in_attn, h_in_attn, h_in_attn, attn_mask=mask)
        attn_output = self.spatial_branch(h_in_attn)
        # B, HW, C = h_in_attn.shape
        # H, W = 145, 145
        # x_flat = h_in_attn.view(B * H * W, 4, 32)
        # x_flat = self.spectral_branch(x_flat)
        # attn_output = x_flat.view(B, HW, C)
        # attn_output = self.spatial_branch(attn_output)
        h = self.attn_hc.depth_connection(mix_h_attn, self.attn_dropout(attn_output), beta_attn)
        mix_h_ffn, beta_ffn, _ = self.ffn_hc.width_connection(h)
        h_in_ffn = self.ffn_norm(mix_h_ffn[..., 0, :])
        ffn_output = self.ffn(h_in_ffn)
        h = self.ffn_hc.depth_connection(mix_h_ffn, self.ffn_dropout(ffn_output), beta_ffn)
        return h, alpha

class SpeImageHyperConnectionBlock(nn.Module):
    def __init__(self, dim: int, n_heads: int, rate: int, layer_id: int, dropout: float = 0.1, dynamic: bool = False):
        super().__init__()
        self.dynamic = dynamic
        self.attn_norm = RMSNorm(dim)
...
class ImageHyperConnectionTransformer(nn.Module):
    def __init__(self, image_size: int, patch_size: int, in_channels: int, num_classes: int, 
        ...
        self.spa_layers = nn.ModuleList([
            SpaImageHyperConnectionBlock(
                dim, n_heads, rate, layer_id, dropout, dynamic=self.dynamic
            ) for layer_id in range(n_layers)])
        
        self.spe_layers = nn.ModuleList([
            SpeImageHyperConnectionBlock(
                dim, n_heads, rate, layer_id, dropout, dynamic=self.dynamic
            ) for layer_id in range(n_layers)])
        ...
        if n_layers > 0 and drop_path > 0:
            ...
            dpr = [0.0 for _ in range(n_layers)]
        self.spa_drop_paths = nn.ModuleList([
            DropPath(dpr[layer_id]) if dpr[layer_id] > 0.0 else nn.Identity()
            for layer_id in range(n_layers)
        ])
        
        self.spe_drop_paths = nn.ModuleList([
            DropPath(dpr[layer_id]) if dpr[layer_id] > 0.0 else nn.Identity()
            for layer_id in range(n_layers)
        ])
    ...
    def forward(self, x: torch.Tensor, return_features=False):
        ...
        spa_H = x.unsqueeze(2).repeat(1, 1, self.expansion_rate, 1)
        spe_H = x.unsqueeze(2).repeat(1, 1, self.expansion_rate, 1)
        for layer, dp in zip(self.spa_layers, self.spa_drop_paths):
            spa_H, alpha = dp(layer(spa_H))
            y = spa_H.mean(dim=2).permute(0, 2, 1).contiguous()  # (B, C, L_keep or L)
        ...
        for layer, dp in zip(self.spa_layers, self.spa_drop_paths):
            spe_H, alpha = dp(layer(spe_H))
            y = spe_H.mean(dim=2).permute(0, 2, 1).contiguous()  # (B, C, L_keep or L)
            if self.mask_ratio == 0 or not self.training or ids_restore is None:
                Y.append(y.view(B, C, self.width, self.height))
            else:
                # 重建被 mask 前的 full-length token 序列，并填充到对应位置
        ...
        #         plt.savefig(f"./maps/abudance_res_{ii}_{jj}.png")
        #         plt.close(fig)
        # 【设计选择】对最终超隐藏状态取平均，这在视觉任务中很常见
        
        weights = self.softmax(self.weights)
        fusion_spa_spe = spa_H * weights[0] + spe_H * weights[1]
        h_final = fusion_spa_spe.mean(dim=2)  # (B, L_keep or L_full, C)
