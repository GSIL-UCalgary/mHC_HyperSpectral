...
class ImageHyperConnectionBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        cluster_num = clustering.shape[-1]
        k = int(0.1 * HW)
        ...
        for cluster_idx in range(cluster_num):
            ...
            selected_indices.append(topk_indices)              # (B, k)
        ...
        # -----------------------------
        # Safe cluster aggregation
        # -----------------------------
        output = torch.zeros_like(attn_output)
        count  = torch.zeros(B, HW, 1, device=attn_output.device)

        for i in range(cluster_num):
            idx = selected_indices[i]                           # (B, k)
            idx_expand = idx.unsqueeze(-1).expand(-1, -1, C)

            x_i = self.spatial_adpative_clustering_branch[i](
                attn_output.gather(1, idx_expand)
            )                                                    # (B, k, C)

            output.scatter_add_(1, idx_expand, x_i)
            # count.scatter_add_(
            #     1,
            #     idx.unsqueeze(-1),
            #     torch.ones_like(idx.unsqueeze(-1), dtype=count.dtype)
            # )
