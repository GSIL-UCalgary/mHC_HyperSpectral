...
def downsample_mode(label, factor=3):
    h, w = label.shape
    new_h, new_w = h // factor, w // factor
    downsampled = np.zeros((new_h, new_w), dtype=label.dtype)

    for i in range(new_h):
        for j in range(new_w):
            # 提取 3x3 窗口
            patch = label[i * factor:(i + 1) * factor, j * factor:(j + 1) * factor]
            # 计算众数
            values, counts = np.unique(patch, return_counts=True)
            downsampled[i, j] = values[np.argmax(counts)]  # 取出现次数最多的值

    return downsampled

def load_data(data_set_name, data_path='./data'):
    if data_set_name == 'IN':
        data = sio.loadmat(os.path.join(data_path, 'IN', 'Indian_pines_corrected.mat'))['indian_pines_corrected']
        labels = sio.loadmat(os.path.join(data_path, 'IN', 'Indian_pines_gt.mat'))['indian_pines_gt']
    if data_set_name == 'UP':
    ...
    elif data_set_name == 'LN':
        data = sio.loadmat(os.path.join(data_path, 'LN', 'LN01_HSI.mat'))['HSI']
        labels = sio.loadmat(os.path.join(data_path, 'LN', 'LN01_GT.mat'))['cdata']
        labels = downsample_mode(labels)
...
def sampling(ratio_list, num_list, gt_reshape, class_count, Flag):
    ...
    val_num = [10] * class_count  # for Indian Pines dataset
